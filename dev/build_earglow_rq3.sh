#!/usr/bin/env bash
# earglow-rq3 -- ear glow with an INSTANCE-MATCH gate. handoff/101 sec 12/13.
#
...
#
#   ./dev/build_earglow_rq3.sh [--install] [--base <skin.set name>]
#
# Nine gates, all offline, then the driver self-test:
#   0 base provenance 77/4/12         5 identity: ctl 93/93, live 10/93
#   1 dis->as byte-neutral            6 verify_earglow_rq3.py + --negative
...
PY="$MOD_DIR/dev/patch_earglow_rq3.py"
VERIFY="$MOD_DIR/dev/verify_earglow_rq3.py"
VERIFY1="$MOD_DIR/dev/verify_earglow_rq.py"
WORK="$MOD_DIR/dev/disasm/earglow_rq3"

BASE=gi-50b-bleed-oil-sheen-deep-clothhi-cone2all-fog
CTL=earglow-rq-ctl
K=0.22
DO_INSTALL=0
# --lineage NAME: build the glow rung a SECOND time under its lineage name, as
# a full rung through every gate. It is NOT a copy of swaps.earglow-rq3 -- it
# is re-derived from the base bytes with the same arguments, and gate 2b then
# demands all 93 modules cmp-identical. That is the proof the lineage name and
# earglow-rq3 are the same shader, not two shaders that happen to be similar.
LIN=""
while (( $# )); do
    case "$1" in
        --install) DO_INSTALL=1 ;;
        --base) BASE="${2:?--base needs a skin.set name}"; shift ;;
        --lineage) LIN="${2:?--lineage needs an output set name}"; shift ;;
...
ORDER=(earglow-rq3-hit earglow-rq3 earglow-rq3-hi)
declare -A RUNG_ARGS=(
    [earglow-rq3-hit]="--k $K --mode hit --wide 4.0 --wrap 0.35"
    [earglow-rq3]="--k $K --wide 4.0 --wrap 0.35"
    [earglow-rq3-hi]="--k $K --wide 6.0 --wrap 0.5"
)
if [[ -n "$LIN" ]]; then
    ORDER+=("$LIN")
    RUNG_ARGS[$LIN]="${RUNG_ARGS[earglow-rq3]}"
    export EG_LINEAGE="$LIN"
fi
for r in "${ORDER[@]}"; do
...
for pair in "earglow-rq3-hit earglow-rq3" "earglow-rq3 earglow-rq3-hi" \
            "earglow-rq3-hit earglow-rq3-hi" "earglow-rq3 earglow-rq2" \
            "earglow-rq3-hit earglow-rq2-hitw"; do
    set -- $pair; d=0
...
echo "  10 of 10 differ between every pair of rungs, and vs earglow-rq2 itself"

if [[ -n "$LIN" ]]; then
    echo "=== 2b. lineage identity: swaps.$LIN vs swaps.earglow-rq3"
    d=0; n=0
    for f in "$MOD_DIR/swaps.$LIN"/*.spv; do
        cmp -s "$f" "$MOD_DIR/swaps.earglow-rq3/$(basename "$f")" || d=$((d+1))
        n=$((n+1))
    done
    [[ "$n" == 93 && "$d" == 0 ]] \
        || { echo "  !! $LIN: $d of $n modules differ from earglow-rq3" >&2; exit 1; }
    echo "  93 of 93 byte-identical -- independently re-derived, not copied"
fi
...
# changed with it.
WANT = {
 'earglow-rq3-hit': dict(mode='hit',  k=0.22, soft=dict(wide=4.0, wrap=0.35)),
 'earglow-rq3':     dict(mode='glow', k=0.22, soft=dict(wide=4.0, wrap=0.35)),
 'earglow-rq3-hi':  dict(mode='glow', k=0.22, soft=dict(wide=6.0, wrap=0.5)),
}
_lin = os.environ.get('EG_LINEAGE')
if _lin:
    # Spelled out again rather than aliased: a lineage build that drifted from
    # earglow-rq3 fails here on its own merits, not by construction.
    WANT[_lin] = dict(mode='glow', k=0.22, soft=dict(wide=4.0, wrap=0.35))
bad, CENSUS = [], None
for r in rungs:
    d = os.path.join(mod_dir, 'swaps.' + r)
    want = WANT[r]
    mods = writes = skipped = 0
    for f in sorted(glob.glob(os.path.join(d, '*.earglowrq3.report.json'))):
        rep = json.load(open(f)); q = rep['earglow_rq3']; mods += 1
        ...
        # Stated here, not read from the report: query C's push past the
        # backface, its tmin, that its tmax IS the module's own sun shadow-ray
        # tmax, that the sun's own cull mask arms are [0, 39] so C sees the
        # sun's occluder set, and that the visibility gate is on.
    ...
    print(f'  {r:16s} 10 modules, {writes} painted writes, {skipped} benign '
          f'skips, A=517 B=545 C=517, tmin=0.0015, tmax=0.018, '
          f'C push=0.001 tmax={q["tmax_c_value"]}, match=InstanceId, '
...
for r in rungs:
    ...
    for f in sorted(glob.glob(os.path.join(d, '*.rgs_reference_main.spv'))):
        ...
        # rq3: THREE queries and three Proceeds, but still only TWO InstanceId
        # reads (A and B) and ONE committed T -- query C answers "is anything
        # in the way" and its identity is irrelevant.
        want3 = 0 if h in PASS else 3
        want2 = 0 if h in PASS else 2
        want1 = 0 if h in PASS else 1
        if (n_i, n_p, n_d, n_t) != (want3, want3, want2, want1):
            bad.append(f'{r}/{h}: init/proceed/instanceId/T = '
                       f'{(n_i, n_p, n_d, n_t)}, want {(want3, want3, want2, want1)}')
        if n_c:
...
echo "=== 6. verify_earglow_rq3.py on the shipped .spv"
python3 "$VERIFY" "$MOD_DIR/swaps.earglow-rq3-hit" --base "$SRC" --mode hit --wrap 0.35
python3 "$VERIFY" "$MOD_DIR/swaps.earglow-rq3"     --base "$SRC" --mode glow --k "$K" --wide 4.0 --wrap 0.35
python3 "$VERIFY" "$MOD_DIR/swaps.earglow-rq3-hi"  --base "$SRC" --mode glow --k "$K" --wide 6.0 --wrap 0.5
[[ -n "$LIN" ]] && python3 "$VERIFY" "$MOD_DIR/swaps.$LIN" --base "$SRC" --mode glow --k "$K" --wide 4.0 --wrap 0.35
...
for dec in noc cullfront invert; do
    patch_set "$WORK/decoy/$dec" --k "$K" --wide 4.0 --wrap 0.35 --decoy "$dec"
done
G=(--base "$SRC" --mode glow --k "$K" --wide 4.0 --wrap 0.35)
reject "--decoy noc (query C traced but never consulted -- i.e. earglow-rq2)" \
       "$VERIFY" "$WORK/decoy/noc" "${G[@]}"
reject "--decoy cullfront (C culls front faces: it would miss the occluder it exists to find)" \
       "$VERIFY" "$WORK/decoy/cullfront" "${G[@]}"
reject "--decoy invert (accept when C HITS: lights exactly the occluded pixels)" \
...
reject "earglow-rq3 read as the -hit diagnostic" \
       "$VERIFY" "$MOD_DIR/swaps.earglow-rq3" --base "$SRC" --mode hit --wrap 0.35
reject "earglow-rq3-hit read as the glow rung" \
       "$VERIFY" "$MOD_DIR/swaps.earglow-rq3-hit" "${G[@]}"
reject "earglow-rq3-hit read with the -hi wrap edge (0.5)" \
       "$VERIFY" "$MOD_DIR/swaps.earglow-rq3-hit" --base "$SRC" --mode hit --wrap 0.5
reject "earglow-rq3-hi read with earglow-rq3's transfer (wide 4.0 / wrap 0.35)" \
       "$VERIFY" "$MOD_DIR/swaps.earglow-rq3-hi" "${G[@]}"
...
if [[ -d "$MOD_DIR/swaps.earglow-rq" ]]; then
    reject "101 sec 2's earglow-rq (ONE query) read as an rq3 rung" \
           "$VERIFY" "$MOD_DIR/swaps.earglow-rq" "${G[@]}"
    reject "earglow-rq3 read by the OLD verify_earglow_rq.py (ONE query)" \
           "$VERIFY1" "$MOD_DIR/swaps.earglow-rq3" "${G[@]}"
fi
...
RUNGS = {'earglow-rq3': 4.0, 'earglow-rq3-hi': 6.0}
if os.environ.get('EG_LINEAGE'):
    RUNGS[os.environ['EG_LINEAGE']] = 4.0
K = 0.22
bad = []
...
for r in "${ORDER[@]}"; do
    dest="$MOD_DIR/swaps.$r"
    {
      ...
      if [[ "$r" == "$LIN" ]]; then
        ...
        echo "#          accept also demands C MISSED (the exit point sees the sun)"
        echo "# SHOT and KEPT (handoff/101 sec 17). This is the lineage name of"
        echo "# earglow-rq3 and is byte-identical to it: it is the SHIPPED DEFAULT."
      else
        echo "# UNSHOT. Read handoff/101 sec 13 BEFORE the screen; the frame must"
        echo "# be BACKLIT, and -rq3-hit must be shot in the SAME frame."
      fi
    } > "$dest/MANIFEST.txt"
done
...
echo
echo "select with skinspec=earglow-rq3-hit / earglow-rq3 / earglow-rq3-hi"
...
echo "          photo mode. Read handoff/101 sec 16 BEFORE the screen and shoot"
echo "          -rq3-hit in the SAME frame as earglow-rq3."
