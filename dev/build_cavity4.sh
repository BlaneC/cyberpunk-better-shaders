#!/usr/bin/env bash
# handoff/90 (second base) -- the cavity cone with 89 sec 2's GATE FIX, on the
# PLAIN standing rung. No bounce floor.
#
# Why this exists: -b3 was shot and REVERTED -- three bounces at 1 spp read as
# visibly noisier, which is the variance cost of an extra stochastic path
# segment (89 sec 5 said it was not a noise FIX; it is in fact a noise SOURCE).
# The gate fix is orthogonal to that and still wanted, so it is rebuilt here on
# the base that is actually live.
#
# The gate: the cone's `== 0` conjunct tests the PATH loop's counter. Pre-89 it
# tested whatever E.find_bounce_counter returned -- the SAMPLE counter in 5 of
# the 12 permutations, correct in the other 7 -- so the cavity darkening ran at
# every bounce in 5 and only at the primary hit in 7, decided at random per
# launch. See 90 sec 1.
#
# THE GATE A/B IS FREE HERE: 88's -cone2 and -cone2all are the old-gate builds
# of these exact rungs and are already parked. -cone2allgf vs -cone2all is one
# variable, the gate.
#
...
WORK="$MOD_DIR/dev/disasm/cavity4_12"
...
RUNG_NAMES=(gi-50b-bleed-oil-sheen-deep-clothhi-cone2gf
            gi-50b-bleed-oil-sheen-deep-clothhi-cone2allgf
            gi-50b-bleed-oil-sheen-deep-clothhi-cone2all35gf)
RUNG_SPECS=("0.85,0.006,2,12,sun,bounce"
            "0.85,0.006,2,12,all,bounce"
            "0.85,0.006,2,12,all,bounce,0.35")
# No -sg control rung is needed on THIS base: 88's existing -cone2 and
# -cone2all ARE the old-gate builds, so -cone2allgf vs -cone2all is already
# the one-variable gate A/B, using rungs that are already parked.
...
K0="$MOD_DIR/dev/disasm/cavity4_k0"
rm -rf "$K0"; mkdir -p "$K0"
...
PYS
    ...
    sed -e "1s/^$BASE_NAME /$name /" \
        -e "1s/ref=12([^)]*)/ref=12(12 cone k=$k tmax=${tmm}mm taps=$taps theta=${theta}deg gate=$gate)/" \
    ...
    done
    {
        ...
        echo "# Base is the plain standing rung -- NO bounce floor (89 was reverted)."
