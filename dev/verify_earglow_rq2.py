#!/usr/bin/env python3
"""verify_earglow_rq2.py <rung-dir> --base <base-dir> --mode glow|hit|hitw
                         [--k K] [--wide W] [--wrap R]
   verify_earglow_rq2.py --negative <base-dir>

Re-derives the earglow-rq2 splice from the SHIPPED .spv bytes -- never from the
...
Proven per patched rgs_reference_main permutation:
  ...
  1  RayQueryKHR capability + extension; ONE OpTypeRayQueryKHR; exactly TWO
     Function-storage ray query variables, both inside the entry block's
  ...
  3  exactly TWO OpRayQueryInitializeKHR, on the two declared objects, sharing
     ONE cull mask -- OpSelect(gate, 39, 0) -- so both queries are gated
     identically and a non-skin pixel pays two guaranteed misses and no branch;
  ...
  7  exactly TWO Proceed, TWO committed InstanceId getters (one per query),
     ONE committed T getter (query B's), and ZERO of the ten other getters;
  ...
  9  the paint is DOMINATED by that compare: the boolean feeding the k select
...
def check_module(path, base_path, mode, k, wide, wrap):
    ...
    if len(rqv) != 2:
        return bad(name, f"{len(rqv)} ray query variables, want exactly 2 "
    ...
    if len(inits) != 2:
        return bad(name, f"{len(inits)} OpRayQueryInitializeKHR, want exactly 2")
    for q in inits:
        if len(q) != 8:
    ...
    if sorted(x for x in byflags if x is not None) != [FLAGS_A, FLAGS_B] or \
            any(len(v) != 1 for v in byflags.values()):
        return bad(name, f"query flag words are {sorted(byflags)}, want exactly "
                         f"one {FLAGS_A} (query A) and one {FLAGS_B} (query B)")
    qA, qB = byflags[FLAGS_A][0], byflags[FLAGS_B][0]
    if {qA[0], qB[0]} != set(rqv):
        bad(name, "the two queries do not run on the two declared objects")
    if qA[3] != qB[3]:
        bad(name, f"the two queries use different cull masks ({qA[3]} vs "
                  f"{qB[3]}) -- they must share ONE gate")
    if qA[1] != nee[0] or qB[1] != nee[0]:
    ...
    # ---- 7. two Proceeds, two InstanceIds, one T, nothing else ------------
    npro = count(lines, 'OpRayQueryProceedKHR')
    if npro != 2:
        bad(name, f"{npro} x OpRayQueryProceedKHR, want exactly 2 (one per "
    ...
    if len(ity) != 2:
        bad(name, f"{len(ity)} committed GetIntersectionTypeKHR, want 2")
    ...
    elif {q for _, q in ids} != set(rqv):
        bad(name, "the two InstanceId reads are not one per query object")
    ...
    elif len(inits) == 2 and tg and qB[0] != \
    ...
    # ---- 8. THE ONE NEW VARIABLE: the instance compare --------------------
    same = None
    if len(ids) == 2:
        a, b = ids[0][0], ids[1][0]
    ...
    if mode == 'glow':
        ...
        # hit and hitw share EVERY check below; hitw differs only in that the
        # flat paint is multiplied by the glow rung's own wrap envelope first.
        want_ss = 1 if mode == 'hitw' else 0
        ...
        if mode == 'hitw':
            ...
            if not wrap:
                bad(name, "--mode hitw needs --wrap to check against")
            else:
                ...
                if not any(v is not None and close(v, wrap) for v in got):
                    bad(name, f"the added SmoothStep's edge is {got}, not the "
                              f"glow rung's wrap {wrap} -- -hitw must use the "
                              f"GLOW rung's envelope, not its own")
        for v in (0.32, 0.04):
        ...
        # -hitw inserts exactly one multiply (by the wrap) between the select
        ...
        if mode == 'hitw' and len(wrapped) != 3:
            bad(name, f"{len(wrapped)} of the 3 flat paints are multiplied by "
                      f"the added SmoothStep -- -hitw's whole claim is that its "
                      f"map is the GLOW's paintable set (101 sec 15)")
...
def main():
    ...
    if a.negative:
        ...
        for p in sorted(glob.glob(os.path.join(a.rung, '*.rgs_reference_main.spv'))):
            ...
            n += 1
            tot += r or 0
        print(f"verify_earglow_rq2: {n} permutations, {tot} painted writes, "
